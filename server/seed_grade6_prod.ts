...
const schoolId = 'f071fa58-1731-4779-b16e-d59c720f4776'; // ZAWADI JUNIOR ACADEMY
const branchId = '8b152747-5663-4a5c-b02c-2bbd1e77fb39';
const recorderId = '6a5e40a4-194f-42ab-ae2b-27985c9dc6f8'; // Rico kariuki (Validated)
...
const studentScores = [
    ...
    { "imageName": "FARHAN ALI", "dbName": "FARHAN ALI ABDULLAHI", "scores": { "MATH": 70, "ENG": 54, "KIS": 56, "SCI": 72, "SS": 83, "CA": 68, "IRE": 67, "AGRI": 88 } },
...
async function main() {
    console.log('Starting Production Grade 6 score seeding...');

    // 1. Fetch Grade 6 students from PROD
    ...
    for (const subject of subjects) {
        ...
        if (!test) {
            test = await prisma.summativeTest.create({
                data: {
                    ...
                    schoolId,
                    branchId,
                    testType: 'OPENER' as any
                }
            });
        } else {
            // Ensure existing tests are updated to OPENER
            await prisma.summativeTest.update({
                where: { id: test.id },
                data: { testType: 'OPENER' as any }
            });
        }
        ...
        for (const record of studentScores) {
            ...
            const score = (record.scores as any)[subject.key];
