...
async function matchGrade5StudentsToDatabase() {
  try {
    console.log('\n' + '═'.repeat(130));
    console.log('📚 MATCHING GRADE 5 STUDENTS FROM DATABASE');
    ...
    // Filter for Grade 5 students
    const grade5FromDB = records.filter(r => {
      const classValue = String(r.Class).trim().toLowerCase();
      return classValue === '5' || classValue === 'grade 5' || classValue === 'class 5';
    });

    console.log(`✅ Found ${grade5FromDB.length} Grade 5 students in database\n`);

    if (grade5FromDB.length === 0) {
      console.error('⚠️ No Grade 5 students found in database');
    ...
    // Get all Grade 5 learners from our database
    ...
    console.log(`✅ Found ${grade5Learners.length} Grade 5 learners in system\n`);

    // Convert to matchable format
    const dbStudents = grade5FromDB.map(r => ({
    ...
    for (const learner of grade5Learners) {
    ...
    console.log('─'.repeat(130));
    console.log(`✅ Successfully matched: ${matchCount}/${grade5Learners.length}`);
    console.log(`⚠️ Not matched: ${noMatchCount}/${grade5Learners.length}`);
    console.log(`📈 Match rate: ${((matchCount / grade5Learners.length) * 100).toFixed(1)}%`);
    console.log('═'.repeat(130) + '\n');
