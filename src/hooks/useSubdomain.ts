 ...
 * 
 * Returns:
 * - subdomain: string | null - The detected subdomain or null
 * - accessMode: 'subdomain' | 'path' | 'unknown' - How user accessed
 * - getSchoolDomain: (subdomain: string) => string - Build full domain URL
 * - isTenantAccess: boolean - Whether subdomain-based access is active
 */
export function useSubdomain() {
   ...
   */
  const getAccessMode = useCallback((): 'subdomain' | 'path' | 'unknown' => {
   ...
   */
  const getSchoolIdFromUrl = useCallback((): string | null => {
    const subdomain = getSubdomain();
    if (subdomain) {
   ...
   */
  const getSchoolDomain = useCallback((subdomain: string): string => {
   ...
   */
  const getSchoolPathUrl = useCallback((subdomain: string, path: string = ''): string => {
...
export function useSchoolBranding() {
  ...
  const getBrandingUrl = useCallback((): string | null => {
...
export function useSubdomainCheck() {
  ...
  const suggestSubdomain = useCallback(async (schoolName: string) => {
    try {
